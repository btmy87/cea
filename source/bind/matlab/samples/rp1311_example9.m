...
num_pts = cea.rocket_solution_get_size(soln);
...
cea.rocket_solver_solve_fac(solver, soln, weights, pc, pip, 3, subar, 0, supar, 3, 0, hc, true, ac_at, false, 0.0, false);
...
isp_vac     = cea.rocket_solution_get_property(soln, "CEA_ISP_VACUUM"           , num_pts);
...
fprintf("P, bar           ");
for i = 1:num_pts
    fprintf(" %9.3f ", pressure(i));
end
fprintf("\n");

fprintf("T, K             ");
for i = 1:num_pts
    fprintf(" %9.2f ", temperature(i));
end
fprintf("\n");

fprintf("gamma_s          ");
for i = 1:num_pts
    fprintf(" %9.2f ", gamma(i));
end
fprintf("\n");
...
fprintf("Ae/At            ");
for i = 1:num_pts
    fprintf(" %9.2f ", area_ratio(i));
end
fprintf("\n");

fprintf("C*               ");
for i = 1:num_pts
    fprintf(" %9.2f ", cstar(i));
end
fprintf("\n");
